...
test('all NFL teams plus None; picking one reports id and name', async () => {
  ...
  render(<FavoriteButton teams={NFL_TEAMS} value={NFL_TEAMS[2] ?? null} onChange={onChange} />)
...
test('college lists every FBS team, including unranked Maryland', () => {
  render(<FavoriteButton teams={FBS_TEAMS} value={null} onChange={vi.fn()} />)
...
test('the star is filled while a team is pinned', () => {
  const { container, rerender } = render(
    <FavoriteButton teams={NFL_TEAMS} value={null} onChange={vi.fn()} />,
  )
